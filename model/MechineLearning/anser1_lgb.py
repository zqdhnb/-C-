...
def main():
    ...
    for file_name in train_files:
        ...
        df = get_more_data(df, 65, 0.5)  # 扩充数据
    ...
    lgr = LGBMRegressor(learning_rate=0.0596, boosting_type='gbdt', objective='regression_l1', n_estimators=1000,
                        min_child_samples=168, min_child_weight=0.11, num_leaves=60, max_depth=10, n_jobs=-1,
    ...
    for train_index, val_index in kf.split(train_x):
        ...
        # 训练模型，并在验证集上进行早停
        # xgr.fit(fold_train_x, fold_train_y, eval_set=[(fold_val_x, fold_val_y)], eval_metric='mae', verbose=30,
        #         early_stopping_rounds=15, )
        lgr.fit(fold_train_x, fold_train_y, eval_set=[(fold_val_x, fold_val_y)], eval_metric='mae', verbose=30,
                early_stopping_rounds=15, )

        # 在验证集上进行预测
        # val_pred = xgr.predict(fold_val_x)
        val_pred = lgr.predict(fold_val_x)
        list_1 = []  # 统计大于7.8的个数
        list_true = []
        # 计算准确率
